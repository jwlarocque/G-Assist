...
Copy `home-assistant-plugin.exe`, `manifest_template.json`, `manifest.json`, and `config.json` from the `G-Assist/plugins/home-assistant/dist/home-assistant` folder (inside the repo folder - these were generated by `build.bat`) into `%programdata%\NVIDIA Corporation\nvtopps\rise\plugins\home-assistant`.
...
In accordance with the G-Assist Hackathon requirements, I have included a pre-built executable `home-assistant-plugin.exe` in this repo.  While it does work, I recommend you follow the build instructions above in order to ensure your config is working and to generate a manifest appropriate for your Home Assistant installation.

If you wish to use the provided executable, you will still need to create and fill out a `config.json` and you should then use the `refresh_manifest` command as described in "Registering new devices" above to populate the manifest.
